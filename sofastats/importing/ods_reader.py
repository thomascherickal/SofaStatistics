...
XML_TYPE_PERC = 'percentage'
XML_TYPE_CURRENCY = 'currency'
XML_TYPE_FRACTION = 'fraction'
XML_TYPE_BOOL = 'boolean'
XML_TYPE_TIME = 'time'
XML_TYPE_DATE = 'date'
XML_TYPE_SCIENTIFIC = 'scientific'
RAW_EL = 'raw element'
ATTRIBS = 'attribs'
COLS_REP = 'number-columns-repeated'
ROWS_REP = 'number-rows-repeated'
VAL_TYPE = 'value-type'
VALUE = 'value'
DATE_VAL = 'date-value'
FORMULA = 'formula'
...
def get_contents_xml_tree(filename, lbl_feedback=None,
        progbar=None, prog_step1=None, prog_step2=None):
    debug = False
    myzip = zipfile.ZipFile(filename)
    ...
    if progbar is not None:
        progbar.SetValue(min(prog_step1, mg.IMPORT_GAUGE_STEPS))
    if lbl_feedback is not None:
        lbl_feedback.SetLabel(
            _('Please be patient. The next step may take a few minutes ...'))
    wx.Yield()
    tree = etree.parse(cont)  ## the most time-intensive bit
    if lbl_feedback is not None: lbl_feedback.SetLabel('')
    wx.Yield()
    if progbar is not None:
        progbar.SetValue(min(prog_step2, mg.IMPORT_GAUGE_STEPS))
    wx.Yield()
...
def after_end_brace(text):
    try:
        ...
        return Exception, f'No curly brace in text: {text}'
...
def get_streamlined_attrib_dict(items):
    ...
    debug = False
    large = False
    if debug and not large: print(f'items: {items}')
    new_attrib_dict = {}
    for item in items:
        key, value = item
...
def get_tbl(tree):
    ...
    for el in sheet:
        if el.tag.endswith('table'):
            ...
            break
    if tbl is None:
        raise Exception('Unable to get tree from xml')
    return tbl
...
def get_has_data_cells(el):
    ...
    if not el.tag.endswith('table-row'):
        raise Exception('Trying to check for data cells but not a table-row')
    for sub_el in el:  ## only interested in table cells. Look in them for data value.
...
def get_ok_fldnames(tbl, rows_to_sample, *,
        has_header=True, headless=False, force_quickcheck=False):
    """
    ...
    if has_header:
        ...
        try:
            ...
            ok_fldnames = get_fldnames_from_header_row(row,
                headless=headless, force_quickcheck=force_quickcheck)
        except IndexError:
        ...
        for i, row in enumerate(rows):
            ...
            for el in row:
                ...
                if not el.tag.endswith('table-cell'):
                    continue
                el_attribs = get_streamlined_attrib_dict(el.attrib.items())
                if VAL_TYPE in el_attribs:  ## real data value(s) prev ns are counted iff followed by data cell(s)
...
def get_fldnames_from_header_row(row, *,
        headless=False, force_quickcheck=False):
    """
    ...
    for el in row:
        ...
        if DATE_VAL in el_attribs:
            ...
            fldnames = [el_attribs[DATE_VAL], ]*colrep  ## take proper date val e.g. 2010-02-01 rather than orig text of 01/02/10
    ...
    ## If last fldname is empty, remove it. We had to give it a chance as a potential divider column but it clearly wasn't.
    if orig_fldnames[-1] == '':
        orig_fldnames.pop()
    if debug: print(orig_fldnames)
    processed_fldnames = importer.process_fldnames(
        orig_fldnames, headless=headless, force_quickcheck=force_quickcheck)
    if debug: print(processed_fldnames)
    return processed_fldnames

def get_ods_dets(progbar, tbl, fldnames, faulty2missing_fld_list,
        prog_steps_for_xml_steps, next_prog_val, *,
    ...
    for (key, value) in tbl.attrib.items():
        if key[-5:].endswith('}name'):
            if debug: print(f"The sheet is named '{value}'")
    datarows = []
    col_type_sets = []  ## one set per column containing all types in column
    fldtypes = {}
    if debug:
        print(f'prog_steps_for_xml_steps: {prog_steps_for_xml_steps}')
        print(f'next_prog_val: {next_prog_val}')
    ...
    if (has_header and row_n == 1) or (not has_header and row_n == 0):
        raise Exception('No data rows identified. Either there were no '
            'data rows at all or there were too many empty rows before the '
            'first data row.')
    steps_per_item = prog_steps_left/row_n
    if debug: print(f'Has {row_n:,} rows including any headers')
    ...
    for row_num, row in enumerate(rows, 1):
        try:
            if has_header and row_num == 1:
                continue
            col_type_sets, valdict = dets_from_row(
                fldnames, col_type_sets, fld_first_mismatches, row, row_num)
            if valdict:
                datarows.append(valdict)
            gauge_val = min(
                next_prog_val + ((row_num-1)*steps_per_item),
                mg.IMPORT_GAUGE_STEPS)
            progbar.SetValue(gauge_val)
            wx.Yield()
        except Exception as e:
            raise Exception(f'Error getting details from row {row_num:,}.'
                f'\nCaused by error: {b.ue(e)}')
    """
    2) Get user decisions on each field where there are potentially more than
    one data type.
    """
    for fldname, type_set, first_mismatch in zip(
            fldnames, col_type_sets, fld_first_mismatches):
        fldtype = importer.get_best_fldtype(fldname, type_set,
            faulty2missing_fld_list, first_mismatch, headless=headless)
...
def update_types_and_valdict(
        col_type_sets, col_idx, fldnames, valdict, coltype, val2use):
    """
    ...
    try:
        ...
        raise Exception(
            "Didn't get enough field names to cover all the data cells.")
    ...
    for unused in range(get_col_rep(attrib_dict)):
        ...
        if not mismatch_in_col:  ## sniff for the first (not interested in subsequent)
            ...
            if n_types_in_col == 2:  ## can't all match - now more than one type
                main_type_set.discard(coltype)  ## leaving only the expected type given we have the one just added and the previous which we can treat as the expected type.
                expected_fldtype = list(main_type_set)[0]
                mismatch_in_col = importer.FIRST_MISMATCH_TPL.format(
                    row=row_num, value=val2use, fldtype=expected_fldtype)
                fld_first_mismatches[col_idx] = mismatch_in_col  ## will never collect mismatch for this col again
            elif n_types_in_col > 2:
                raise Exception('Should have identified first mismatch when '
                    'reached two non-missing col types but here we are with '
                    'three. Something must be wrong.')
...
def get_val_and_type(attrib_dict, el_det):
    ...
    if xml_type == XML_TYPE_STR:
        ...
        try:
            ...
            google_docs_default_date = '%m/%d/%Y'  ## OK if already in list
            ...
            try:  ## so we don't assume 2136 is a year
                probable_date_formats.remove('%Y')
            except ValueError:
...
def get_vals_from_row(row, n_flds):
    ...
    for el_det in tbl_cell_el_dets:
        if got_enough:
            break
        attrib_dict = el_det[ATTRIBS]  ## already streamlined the defaults unless overridden by actual data
        val2use = None
...
def dets_from_row(fldnames, col_type_sets, fld_first_mismatches, row, row_num):
    ...
    col_idx = 0  ## Note may be several tbl_cell_el_dets in a single col so can't just enumerate. May be incremeneted inside process_cells because of col-repeat
    for el_det in tbl_cell_el_dets:  ## moving across row through col cells (Note - some cells span multiple cols if col-repeat)
        ...
        if DATE_VAL in attrib_dict:
            ...
            val2use = attrib_dict[DATE_VAL]  ## take proper date value e.g. 2010-02-01 rather than orig text of 01/02/10
            if debug: print(val2use)
            ## We pass over to process_cells which handle col-repeats. At the end of that process we need to know what col_idx we are on. 
            ...
            col_idx = process_cells(attrib_dict, col_type_sets, 
                ...
                coltype=mg.VAL_EMPTY_STRING, val2use='', row_num=row_num)
        else:
            pass
        idx_final_col = len(fldnames) - 1
        if col_idx > idx_final_col:
