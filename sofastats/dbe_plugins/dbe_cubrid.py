...
def set_data_con_gui(parent, scroll, szr, lblfont, *, read_only=False):
    ...
    ## default database
    szr_cubrid_inner_top.Add(
        parent.lbl_cubrid_default_db, 0, wx.LEFT|wx.RIGHT, 5)
    szr_cubrid_inner_top.Add(
        parent.txt_cubrid_default_db, 0, wx.GROW|wx.RIGHT, 10)
    ## default table
    szr_cubrid_inner_top.Add(
        parent.lbl_cubrid_default_tbl, 0, wx.LEFT|wx.RIGHT, 5)
    szr_cubrid_inner_top.Add(
        parent.txt_cubrid_default_tbl, 0, wx.GROW|wx.RIGHT, 10)
...
def set_con_det_defaults(parent):
    try:
        ...
        parent.cubrid_default_db = 'demodb'
    try:
...
def process_con_dets(parent, default_dbs, default_tbls, con_dets):
    ...
    if ':' in raw_host:
        cubrid_host, raw_port = raw_host.split(':')
        try:
    ...
    if has_cubrid_con:
        con_dets_cubrid = {
            'host': cubrid_host, 'port': cubrid_port, 
