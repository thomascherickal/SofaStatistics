...
#
# Matplotlib canvas in a top-level wxPython dialog.
#

class PlotDlg(wx.Dialog):
    """
    Based on Ken McIvor's wxmpl PlotFrame class.
    A matplotlib canvas embedded in a wxPython dialog.
    size is in inches
    Fit() etc is delayed so buttons can be added to sizer later. NB add 
        buttons to self not self.panel otherwise overlaid on plot ;-).
    """
    def __init__(self, parent, title, size=(6.0, 3.7), dpi=96, cursor=True,
            location=True, crosshairs=True, selection=True, zoom=True,
            autoscaleUnzoom=True, **kwds):
        ...
        self.sizer.Add(self.panel, 1, wx.ALL|wx.GROW, 5)
